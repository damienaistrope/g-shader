...
export const Dialog: React.FC<DialogProps> = ({
  ...
  const content = (
    <motion.div
      className={`md-dialog ${scrollable ? 'md-dialog--scrollable' : ''} ${isStatic ? 'md-dialog--static' : ''} ${className}`}
      style={style}
      initial={isStatic ? false : { opacity: 0, scale: 0.9, y: 20 }}
      animate={isStatic ? { opacity: 1, scale: 1, y: 0 } : { opacity: 1, scale: 1, y: 0 }}
